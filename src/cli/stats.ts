  ...
  try {
    ...
    if (!showProjectOnly) {
      ...
      if (session) {
        ...
        if (input.transcript_path) {
          ...
          lines.push(`- **Utilization:** ${bar} ${pct}% (~${formatTokens(util.estimatedTokens)} tokens)`);

          if (session.context_utilization_peak) {
            lines.push(`- **Peak utilization:** ${(session.context_utilization_peak * 100).toFixed(1)}%`);
          }
        }
      } else {
      ...
      if (input.transcript_path) {
        ...
        if (util.utilization >= 0.7) {
          ...
          lines.push("Context is filling up. Consider compacting soon or being concise.");
        } else {
          lines.push("Context utilization is healthy. Continue working.");
        }
      } else {
        lines.push("Transcript path not available — cannot estimate utilization.");
      }
      lines.push("");
    }
    ...
    if (!showSessionOnly) {
      ...
      if (pm.avgUtilization > 0) {
        lines.push(`- **Avg peak utilization:** ${(pm.avgUtilization * 100).toFixed(1)}%`);
      }
      lines.push(`- **Total compactions:** ${pm.totalCompactions}`);
      if (pm.sessionCount > 0) {
        lines.push(`- **Compaction frequency:** ${(pm.totalCompactions / pm.sessionCount).toFixed(1)} per session`);
      }
      ...
      if (learnings.length === 0) {
        ...
        for (const l of learnings.slice(0, 10)) {
          ...
          const refs = l.times_referenced > 0 ? ` (ref: ${l.times_referenced}x)` : "";
          lines.push(`- ${cat} ${l.learning.slice(0, 120)} — relevance: ${score}${refs}`);
        }
